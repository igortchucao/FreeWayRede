...
public class Vehicles implements Serializable {
	...
	public void tick() { 
		dados.setPOS_X(dados.getPOS_X() + dados.SPEED);
		if (dados.getPOS_X() > 1400)
			dados.FAIXA = false;
	}
	...
	public void render(Graphics g, int faixa) {
		if (dados.COR < 5)
			...
			if (dados.MAO == 1) {
				g.drawImage(vehicleImage, (int) dados.getPOS_X(), 80 + faixa * 59, -128, 64, null);
			} else {
				g.drawImage(vehicleImage, (int) (1400 - dados.getPOS_X()), 80 + faixa * 59, 128, 64, null);
			}
		} 
		if(dados.COR != 7){
			if (dados.MAO == 1) {
				g.drawImage(vehicleImage,(int) dados.getPOS_X(), 80 + faixa * 59, -64, 64, null);
			} else {
				g.drawImage(vehicleImage,(int) (1400 - dados.getPOS_X()), 80 + faixa * 59, 64, 64, null);
			}
		}
	}
}
